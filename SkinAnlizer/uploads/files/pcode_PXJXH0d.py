...
def vec2mat_3D_reshape(vec, row, col):
		
	if (len(vec) != 3*row*col):
		print 'error of vector length'
		exit()
	
	tmp = np.zeros((3, row, col))
	vec_indx   = row*col
	first_vec  = vec[0:vec_indx]
	second_vec = vec[vec_indx:(2*vec_indx)]
	third_vec  = vec[(2*vec_indx):(3*vec_indx)]
	
	tmp[0, :, :] = np.reshape(first_vec, (row, col), order='F')
	tmp[1, :, :] = np.reshape(second_vec, (row, col), order='F')
	tmp[2, :, :] = np.reshape(third_vec, (row, col), order='F')
...
def run_sc(patches, numBases, iters, lambd):
	...
	for i in range(iters):
		...
		dictionary = l2l_learn_basis_dual(patches, S)
...
def extract_feature(X, D, rfSize, SKIN_DIM, M, W, lambd):
	...
	for i in range(X.shape[0]):
		...
		first_tmp  = im2col(np.reshape(X[i, 0:length_RGB], SKIN_DIM[0:2]),   [rfSize, rfSize])
		second_tmp = im2col(np.reshape(X[i, length_RGB:(2*length_RGB)]),     [rfSize, rfSize])
		third_tmp  = im2col(np.reshape(X[i, (2*length_RGB):(3*length_RGB)]), [rfSize, rfSize])		

		#np.vstack 바꿔
		patches = np.hstack((first_tmp, second_tmp, third_tmp))
		patches = patches.T

		tmp1    = np.var(patches, 1) * patches.shape[1] / (patches.shape[1] - 1)
	        tmp1    = np.sqrt(tmp1 + 10)
       		tmp2    = np.mean(patches, 1)
       	 	tmp_mat = zeros(patches.shape[0], patches.shape[1])
		...
		patches = np.reshape(patches, [2*numBases, prows, pcols])
...
def run():
	...
	for indx in range(1, numPatches+1):
			
		if ((i % 1000) == 0):
			print 'Extracting patch: ', indx, '/', numPatches
		r = random.randint(0, SKIN_DIM[0] - rfSize)
		C = random.randint(0, SKIN_DIM[1] - rfSize)
		tmp = trainX[random.randint(0, trainX.shape[0]), :]
